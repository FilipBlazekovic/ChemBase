...
@Path("/users/delete")
public class EndpointUserDelete extends ResourceConfig {
    ...
    public Response deleteUser(@CookieParam("sessionID") Cookie sessionCookie, JSONUserManagementRequest request)
    {
        ...
        logger.info("ENDPOINT: /users/delete");
        ...
        if (VALIDATION_STATUS)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        logger.info("SessionID valid!");
                        logger.info("Checking if specified user can be deleted...");
                        statement = connection.prepareStatement(SQLStatements.CHECK_IF_DELETABLE_USER_SQL);
                        statement.setInt(1, userID);
                        resultSet = statement.executeQuery();

                        if (resultSet.next())
                        {
                            logger.info("Action not allowed => Admin user can't be deleted!");
                            RETURN_MESSAGE = HTTPResponseMessages.HTTP_FORBIDDEN;
                        }
                        else
                        {
                            ...
                            statement = connection.prepareStatement(SQLStatements.DELETE_USER_SQL);
                            statement.setInt(1, userID);
                            statement.execute();
