...
@Path("/locations/add")
public class EndpointLocationAdd extends ResourceConfig {
    ...
    public Response addLocation(@CookieParam("sessionID") Cookie sessionCookie, JSONLocationManagementRequest request)
    {
        ...
        logger.info("ENDPOINT: /locations/add");
        logger.info("Retrieved params:");
        logger.info("sessionID: " + (sessionID != null ? sessionID : ""));
        logger.info("location: "  + (location != null ? location : ""));
        ...
        else if (location == null || location.length() == 0)
        {
            ...
            logger.info("Validation fail => missing location!");
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        logger.info("Checking if location already exists...");
                        statement = connection.prepareStatement(SQLStatements.CHECK_IF_DUPLICATE_LOCATION_SQL);
                        statement.setString(1, location);
                        resultSet = statement.executeQuery();
                        
                        if (resultSet.next())
                        {
                            logger.info("Action not allowed => Location already exsits!");
                        ...
                        else
                        {
                            ...
                            statement = connection.prepareStatement(SQLStatements.ADD_LOCATION_SQL);
                            statement.setString(1, location);
                            statement.execute();
