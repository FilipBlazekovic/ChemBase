...
@Path("/users/add")
public class EndpointUserAdd extends ResourceConfig {
    ...
    public Response addUser(@CookieParam("sessionID") Cookie sessionCookie, JSONUserManagementRequest request)
    {
        ...
        logger.info("ENDPOINT: /users/add");
        ...
        else if (username.toUpperCase().equals("ADMIN"))
        {
            ...
            logger.info("Validation fail => Invalid username!");
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        logger.info("Checking if specified user already exists...");
                        statement = connection.prepareStatement(SQLStatements.CHECK_IF_DUPLICATE_USER_SQL);
                        statement.setString(1, username);
                        resultSet = statement.executeQuery();
                        
                        if (resultSet.next())
                        {
                            logger.info("Action not allowed => User already exists in the database!");
                            RETURN_MESSAGE = HTTPResponseMessages.HTTP_FORBIDDEN;
                        }
                        else
                        {
                            ...
                            statement = connection.prepareStatement(SQLStatements.ADD_USER_SQL);
                            statement.setString(1, username);
